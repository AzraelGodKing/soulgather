...
**v6.10.0.** Cost-reduction prestige (AZR-181 option 2): Favor lowers core producer `COST_MULT` from `1.15` toward `1.06`; soft compound `prestigeMult`; Crown soft-caps after level 8 with `1.1^n` bonus; `namesCompleteMult` 1.05→1.5. Live: https://azraelgodking.github.io/soulgather/. Footer/CSS **v6.10.0**. Save key `soulgather-v0`.

**v6.9.1.** Table-driven `GIFTS` table replaces ~580 lines of hand-written gift branches in `tryMilestoneGifts` with a single loop over `{ flag, stat, at, give, id, toast }` rows; early-out when all gifts granted; stat-indexed evaluation; `bumpPeak*` once per pass; `bonus*` flags renamed to `gift*` with migration for old saves (AZR-179). Declarative `FIELDS` table replaces keptX Tribute locals and five hand-maintained persistence lists (AZR-178); CI workflow + rewritten first-run sim against the live engine (AZR-177); `GAME_VERSION` single source; `package.json`; `no-redeclare`/`no-dupe-keys` ESLint in CI. Stay v6.9.1; hold AZR-191+. Ash rites geometric costs (AZR-162); hidden-tab credit via `simulatedUntil` (AZR-163); live-frame clamp + 1Hz hidden heartbeat + Hollow freeze (AZR-164); load-failure keeps raw save + bak1/bak2 (AZR-165); debounce save + drop per-click full render (AZR-167); save-field sanitise + NaN tripwire (AZR-168); shadowed renderNames removed (AZR-169); unlock-card reveals unified (AZR-170); Quiet Court Tribute ash-autobind deduped after edict stock (AZR-171); buy-mode hotkeys 1/2/3 honor `otherButton` (AZR-172); nextGoal real Favor thresholds (AZR-173); toast queue cap/coalesce/dismiss (AZR-174); Stillness disables Draw + HUD vow chip + swear confirm + Ember greys Night/Wake (AZR-175); closed-form Max buys + throttle render (AZR-166). Live: https://azraelgodking.github.io/soulgather/. Footer/CSS v6.9.1. Save key `soulgather-v0`.
...
**v6.8.** Binding Toll — capped ash side path (rate up, Shade/Spirit costs up). Live: https://azraelgodking.github.io/soulgather/. Footer/CSS v6.8. Save key `soulgather-v0`.

**v6.7.** Autobind Obelisks, Quiet Court Obelisks, and a peak-five-obelisks gift. Footer/CSS v6.7. Save key `soulgather-v0`.
...
**AZR-118 (steepen ash ladder).** After Pyres: Urn/Hearth/Beacon/Spire/Obelisk cost mult `1.2` → `1.28` (bases stay 3/4/4/5/6); unlock gates `UNLOCK_URNS`/`HEARTHS`/`BEACONS`/`SPIRES`/`OBELISKS` `4` → `6` (still OR owned ≥ 1). Chalices: `CHALICE_COST_BASE` 20→32, `CHALICE_COST_MULT` 1.5→1.65 (`CHALICE_MAX` 12). Peak Beacon/Spire/Obelisk gifts: 12→7 ash (toast/Chronicle "seven"). Leaves AZR-117 Well Depth alone; early Shade→Spirit←Vessel→Censer→Pyre unchanged. No footer bump; save key unchanged.

**AZR-119 (Binding Toll).** Optional this-run ash side path after 5 Fetters + first Favor. Cap 4. Spend ash `N.cost(40, 1.45, n)`. Rate: Shade souls/s and Spirit shades/s × `1.12^n`. Shade/Spirit buy costs × `(1 + 0.15×n)` on shadeCost/spiritCost, purchasePlan (manual 1/10/Max), and card labels. No Autobind, no Reliquary edict, no hotkey. Look locked; footer/CSS **v6.8**. Leaves AZR-117 Well curve and AZR-118 ash-ladder numbers untouched. Save key unchanged.

**AZR-120 (soft-cap this-run rites).** `siphonMult` base `2` → `1.55` (`RITE_MULT_BASE`); levy/cinder/urn/hearth/beacon/spire rites share the same curve. Feel: n1 ≈×1.55 (was ×2), n2 ≈×2.40 (was ×4), n3 ≈×3.72 (was ×8), n5 ≈×9.0 (was ×32). Cost bumps: siphon `N.cost(65, 3, n)` (was 50), levy `N.cost(22, 3, n)` (was 15), CINDER 22 (was 15), URN_RITE 18 (was 12), HEARTH_RITE 20 (was 14), BEACON_RITE 24 (was 16), SPIRE_RITE 26 (was 18). No Obelisk rite. Leaves AZR-117 Well, AZR-118 ash ladder, AZR-119 Binding Toll alone. No Favor prestigeMult / Quiet Court change. Look locked; no footer bump (still v6.8). Save key unchanged.

**AZR-121 (Hollow Hunger).** Mid-run idle pressure after Favor earned ≥ 2 and Pyres unlocked. Live-only timer: 90s grace then a stack every 45s, max 5, −4% production each via `hollowMult(stacks)` on `rateMult` (floor 0.8). **"Live"** = visible-tab play, or the AZR-164 1Hz hidden heartbeat with Hollow **frozen** while `document.hidden` — a hidden tab does **not** accrue Hollow. Manual successful spends clear stacks + idle (`noteHollowManualSpend`); **Autobind does not clear.** Offline catchup does not advance idle or stacks. Quiet `#hollow-status` when stacks ≥ 1; toast once per emptying on first stack: "The well grows hollow." Look locked; footer/CSS **v6.9**. Leaves 117/118/119/120 numbers alone. Save key unchanged.

**AZR-148 (Hollow clear min spend).** A successful manual spend clears Hollow stacks and resets `hollowIdle` only if it meets a min spend for its pricing currency (stock = on hand **before** deduct). Souls: `min(500, max(25, floor(0.02 * currentSouls)))` — soul clear **cap 500**. Ash: `max(5, floor(0.02 * currentAsh))`. Shades: `max(3, floor(0.02 * currentShades))`. Other currencies (favor, remembrance, spirits, vessels, lanterns, …) always clear. Sub-threshold spends still purchase normally but do **not** clear stacks or reset the 90s idle timer — Mode-1 Shade poke (~10 souls on a large stock) no longer wipes Hollow. Timing constants from AZR-121 unchanged; Autobind still never calls `noteHollowManualSpend`. Look locked; no footer bump (still v6.9). Save key unchanged.

**AZR-162 (ash rites geometric costs).** Bug: Cinders/Urn/Hearth/Beacon/Spire charged a **flat** ash cost while granting exponential mult (`1.55^n` via `siphonMult`) → positive feedback → Infinity ash. Fix: geometric `N.cost(base, 2.6, n)` matching Siphon/Levy pattern (Siphon/Levy stay mult 3). Bases keep AZR-120: Cinders 22, Urn 18, Hearth 20, Beacon 24, Spire 26. ×1 buy only (one level per click) like Siphon/Levy — not routed through `purchasePlan` / Buy 10 / Max. Render + hotkeys C/U/H/L/S gate on scaling cost; `noteHollowManualSpend` gets actual spent amount. Leaves Binding Toll (AZR-119), Well Depth (AZR-117), ash ladder (AZR-118), and `RITE_MULT_BASE` 1.55 alone. Look locked; footer/CSS **v6.9.1**. Save key unchanged. Pre-fix saves with absurd rite levels may show inflated ash (no migration).

**AZR-163 (hidden-tab time via simulatedUntil).** Bug: while a tab is hidden-but-open, `requestAnimationFrame` pauses so live `applyDt` stops, but throttled autosave still stamped `lastTick = Date.now()` with no simulation → reopen/reload credited ~0 (silent progress loss). Fix: state field `simulatedUntil` (wall-clock ms the sim has been credited through). Advance **only** inside `applyDt` by the clamped `dt * 1000` actually simulated. `serializeState` / autosave serialize `simulatedUntil` from state (must **not** stamp it to `Date.now()`); may still refresh `lastTick` for compat. Offline/load credit: `offline = (Date.now() - state.simulatedUntil) / 1000`, then `MAX_DT` clamp as today. Migrate: `Number(data.simulatedUntil) || Number(data.lastTick) || Date.now()`. `settleToNow()` credits the wall gap via `applyDt(dt, false)` (production only — no Hollow tick, no live Autobind). Handlers: `visibilitychange` hidden → settle + save; shown → settle + `lastFrame = 0` (+ AZR-115 away toast when gap ≥ `AWAY_SUMMARY_DT` with gains); `pagehide` → settle + save (not `beforeunload`). After catchup, sync `simulatedUntil` to now so capped excess beyond `MAX_DT` is not re-credited. freshState / Tribute / Reset / import set `simulatedUntil` to now. Look locked; stay **v6.9.1** (same bug-fix wave). Save key unchanged. **Follow-up AZR-164:** 1Hz `setInterval` background clock, Hollow freeze while hidden, `LIVE_FRAME_MAX` live-cadence clamp.

**AZR-164 (live-frame clamp + 1Hz hidden heartbeat + Hollow freeze).** Bug: returning from a hidden tab dumped the whole rAF/settle gap through live Hollow/Autobind → instant Hollow ×5 and one Autobind pulse for the whole gap. Fix: (1) `tickHollowHunger` returns immediately while `document.hidden` — no Hollow accrual on a hidden tab; offline `applyDt(..., false)` still does not tick Hollow. (2) `LIVE_FRAME_MAX = 1.0` — in `applyDt(dt, live)` when live, production still uses the full clamped `dt`; Hollow + Autobind cadence use `liveSpan = Math.min(dt, LIVE_FRAME_MAX)` only. (3) While hidden, 1Hz `setInterval(1000)` heartbeat measures real elapsed since last beat, calls `applyDt(span, true)` (production gets full span; live systems capped inside `applyDt`; Hollow frozen). Clear heartbeat on show / pagehide / unload. Autobind ≈1/sec while the heartbeat runs; under browser throttle, fewer pulses OK — do not dump multi-pulses for a long gap on one frame. Integration with AZR-163: hide → settle then start heartbeat from now; show → clear heartbeat, settle remainder, `lastFrame = 0`. Look locked; stay **v6.9.1**. Save key unchanged.

**AZR-165 (load failure keeps raw save).** Bug: `load()` catch did `state = freshState()` then the 5s autosave overwrote `soulgather-v0` — silent total wipe. Fix: on parse/shape failure keep raw bytes, set `loadFailed`, show blocking `#load-fail-notice` (void/gold/crimson); **never** overwrite the primary key from the catch path. `save()` early-returns while `loadFailed` except confirmed Start Fresh (clears flag then writes). Timed backups `soulgather-v0.bak1` (~1 min) and `soulgather-v0.bak2` (~1 h); on failure offer newest parseable backup (prefer bak1) — no silent swap, no full save slots (AZR-190 later). Harden `isSaveShape` / `importMemory`: require finite `souls` + `lifetimeSouls` (number or `{m,e}`); reject non-finite; toast "The memory would not bind." No mid-load `save()` until restore is known-good (one save after successful catchup OK). NaN field sanitise is AZR-168. Look locked; stay **v6.9.1**. Save key unchanged. Hold AZR-166/167 and AZR-172+.

**AZR-168 (sanitise save fields; freeze autosave on NaN).** Untrusted save numbers must not brick a run. `applySaveData` routes every integer count/level through `loadCount(v, max?)` (`Math.floor(Number(v)||0)`; non-finite or `<0` → 0; optional max) and every Num stock through `loadNum` (`N.load` then finite ≥0 else 0). `N.load` is the Num save boundary: NaN / Inf / `{m:NaN}` / `"abc"` / null / undefined / −1 → finite ≥0 Num. `normalize` maps a non-finite mantissa to `{m:0,e:0}`; `fromNumber(-1)` still exists for internal arithmetic. `isSaveShape` / import require well-formed finite `souls` + `lifetimeSouls`; `favorEarned` if present must be finite ≥0 (missing OK for old saves). Import `{souls:{m:null,e:0}, wellDepth:-5, thrones:-20}` is rejected ("The memory would not bind.") and leaves the current save untouched (`souls.m` null is not well-formed). An already-stored save with `wellDepth:-5` (valid souls) sanitises to 0 via `loadCount`. ~1/s tripwire: if core currencies (souls, shades, ash, lifetimeSouls) are non-finite or negative, freeze autosave (`loadFailed`) and reuse the AZR-165 notice — do not silently wipe LS. `save()` still gated on `loadFailed`. Look locked; stay **v6.9.1**. Save key unchanged. Hold AZR-172+ / 166 / 167 / 190.

**AZR-169 (remove shadowed renderNames).** Two top-level `function renderNames` in the IIFE; the first (`open = n >= 1`, sig `"n" + n + (namesComplete ? "c" : "")`) was dead (hoisted, overwritten). Deleted the dead copy. Live predicate stays `n >= 1 || namesComplete` (panel for a clamped count and for `namesComplete`); `dataset.sig` stays `n + ":" + (namesComplete ? "1" : "0")`. `hideNames` / `adoptSave` still hide the panel / clear `dataset.sig` so the live encoding rebuilds. Duplicate top-level-fn scan in test-economy.mjs (pre-fix main failed on `renderNames`). Look locked; stay **v6.9.1**. Save key unchanged.

**AZR-170 (unify unlock-card reveals).** Bug: `boot()` hand-copied reveal list omitted Spires/Obelisks → save with those unlocked hid cards on first paint until a later render bandage. Fix: single `REVEALABLE` table `{ flag, reveal, el }` for every producer card (Well, Lanterns, Spirits, Fetters, Vessels, Thrones, Censers, Pyres, Urns, Hearths, Beacons, Spires, Obelisks, Chalices). `revealUnlockedCards(withToast)` loops it; `adoptSave`, `layTribute`, and `boot` all call `revealUnlockedCards(false)`; `hideUnlockCards` iterates the same table. Removed `classList.contains("is-hidden")` re-reveal bandages from `render()`. Look locked; stay **v6.9.1**. Save key unchanged. Hold AZR-172+.


**AZR-171 (Quiet Court Tribute ash-autobind dedupe).** Bug: `layTribute()` ran the Quiet Court ash-autobind block (Urns/Hearths/Beacons/Spires/Obelisks) twice — once before Edict starting stock (dead/wrong unlock evaluation) and once after (load-bearing). Fix: delete the pre-stock copy; keep a single post-edict evaluation. Tribute restore tail is phased: kept progression → `applyEdictStartingStock()` → `applyAutobindStarts()` (table-driven QC / Smoke / Cinder / Cut / Tending / Gleam / Rise / Draught) → `revealUnlockedCards(false)`. Ash unlock flags (`unlockedAutobindUrns` etc.) must be evaluated after Edict starting stock. Helpers exported on `SoulgatherEconomy` for headless tests. No economy/look change; stay **v6.9.1**. Save key unchanged. Hold AZR-172+.




**AZR-172 (buy-mode hotkeys honor otherButton).** Bug: digit hotkeys `1`/`2`/`3` always called `setBuyMode` even when another (non-Draw) button was focused — stole focus from Tithe/buy/etc. and could dismiss the AZR-116 first-run hint on a suppressed press. Fix: table-driven `HOTKEYS` `{ keys, guard, action }` with shared default guard `!otherButton` (same as letter hotkeys); `ev.key.toLowerCase()` once; `preventDefault` when a buy-mode hotkey is actually handled. Still switch when nothing is focused or Draw/gather is focused; still ignore while typing in Memory (`isTypingTarget`). Hint dismiss only via accepted `setBuyMode` (unchanged: only on real mode change). Letter hotkey gates identical; Space/Enter stay special-cased. No economy/look change; stay **v6.9.1**. Save key unchanged. Hold AZR-173+.

**AZR-173 (nextGoal real Favor thresholds).** Bug: `nextGoal` hardcoded 25000 for every Tribute after the first, lying about the real curve `favorGain = floor(sqrt(v/25000))` where the nth Favor needs `25000*n^2`. Fix: named `FAVOR_SOULS_BASE = 25000` (sole bare `25000` in `js/game.js`); `soulsForFavor(n)` / `nextFavorThreshold(lifetime)`; pre-first X/Y uses the helper; after `favorEarned >= 1` always show progress toward the next Favor (void voice); when Tribute-ready this run, lead with Lay Tribute + Favor waiting + ordinal next threshold + X/Y. No late-game Names/Chalice/Remembrance second goal line. No look/economy retune of the curve itself; stay **v6.9.1**. Save key unchanged. Hold AZR-174+.

**AZR-174 (cap/coalesce/dismiss toast queue).** Unbounded FIFO toasts + 5.2s dwell + no dismiss locked players out for minutes on milestone bursts. Chronicle stays the record; toasts are transient. `showToast(message, groupKey?)`; same-tick gifts in `tryMilestoneGifts` open a coalesce buffer (`beginGiftToastBatch` / `flushGiftToasts`) so `gifts` toasts merge into one void summary (e.g. `The well was generous: +3 names, +26 ash, +250 souls.` or `The well was generous. N gifts.`). `TOAST_QUEUE_MAX = 5` — overflow replaces the last slot with `…and N more.` Click `#toast` → next; Escape → next; Escape again (`toastEscapeArmed`) → clear queue (never while typing in Memory). `TOAST_MS` 5200 for a lone toast; `TOAST_FAST_MS = 1800` when more wait. AZR-115 away-summary still unshifts first; every gift still `markChronicle`. No economy/look change; stay **v6.9.1**. Save key unchanged.

**AZR-166 (closed-form Max buys + throttled render).** Max mode unplayable at huge banks: every rAF `render()` re-ran 15 purchase plans (linear `maxAffordable` + `bulkCost` up to `BULK_CAP=10000`) and wrote ~285 identical `textContent`s. Fix: (1) Throttle full `render()` to ~12 Hz via dirty flag; `applyDt` stays every rAF; hot souls counter + rate still updated every frame. (2) Closed-form `maxAffordable` via `k = floor(log(1 + cur*(m-1)/first) / log(m))` with off-by-one correction; closed-form `bulkCost` via geometric sum `first*(m^k - 1)/(m - 1)`; well piecewise per segment (early d≤5 / late d≥6). Loop refs kept as `maxAffordableLoop`/`bulkCostLoop`/`wellMaxAffordableLoop`/`wellBulkCostLoop`. (3) `setText(el, str)` skips unchanged `textContent` writes; `setTextWriteCount` for tests. (4) `rateMult()` cached once per render, threaded into `soulsPerSec`/`shadeSoulsPerSec`/`shadesPerSec`/`spiritsPerSec`/`ashPerSec` via optional `cachedRm` arg. (5) `BULK_CAP=10000` sanity ceiling on closed form (not a loop bound). Footer/CSS stays **v6.9.1** (v6.10.0 at wave cut). Save key `soulgather-v0` unchanged. No economy retunes. Hold AZR-167+ / a11y 196–199.

**AZR-175 (Stillness Draw visible rule).** Bug: `harvest()` early-returned under Stillness but Draw stayed an interactive well sigil; Space/Enter still called `harvest` and did nothing — players thought the game was broken. Fix: (1) Disable Draw under Stillness — `gatherBtn.disabled`, `aria-disabled`, visible copy + `aria-label` become "The well is still." (verb "The well" / noun "is still."); restore "Draw from the Well" when not Stillness. Space/Enter document keydown honours `gatherBtn.disabled` (belt-and-suspenders with the stillness check). (2) Persistent HUD vow indicator `#vow-status` inside `.souls-display` after `#hollow-status` for all four vows (gold-dim, small uppercase tracking): Stillness — no draws; Poverty — no autobind Thrones; Hunger — tithe ×2; Ember — no Night's Tithe, no Wake. Hidden when no vow; wipes on Tribute. (3) `window.confirm` before every `swearVow` that names the restriction and says "This cannot be undone until Tribute." Cancel aborts before state mutation. (4) Ember: Night's Tithe + Keep the Wake render "Ember holds the night." / "Ember holds the wake." with `aria-disabled` when not already burning. N/W hotkeys still early-return. (5) OUT: no suppressed-click toast. No economy/look change; stay **v6.9.1**. Save key unchanged.


## Loop (short)

Click the well → Shades → Lanterns (half-step) → Bound Spirits → Fetters (half-step) → Vessels → Censers (side) → Pyres (ash half-step) → Urns (ash half-step after Pyres) → Hearths (ash half-step after Urns) → Beacons (ash half-step after Hearths) → Spires (ash half-step after Beacons) → Obelisks (ash half-step after Spires) / Thrones → Chalices (late ash sink at 5 Thrones). Ash feeds Marks, Night's Tithe, The Wake, Thin the Veil, and Chalices. The Toll is a this-run click burst (80 well draws; 40 Souls; paid duration `paidTollSecs` ×2 on clickPower). Choir of Ash this-run (lantern spend; Edict of the Choir starts it). Hymn after Tribute (`45 + 15 * hymnEdictLevel` seconds, ×1.25). Wake after Tribute if Edict of the Wake (`wakeSecs` when level >= 1). Paid Keep the Wake duration `paidWakeSecs`. Procession after Tribute if Edict of the Procession (`processionSecs` when level >= 1). Toll after Tribute if Edict of the Toll (`tollSecs` when level >= 1). Veil after Tribute if Edict of the Veil (`veilSecs` when level >= 1). Night's Tithe after Tribute if Edict of Night (`nightEdictSecs` when level >= 1). Rites (Siphon / Levy / Binding Toll / Rite of Cinders / Rite of the Urn / Rite of the Hearth / Rite of the Beacon / Rite of the Spire), Tithe, Autobind Shades / Autobind Spirits / Autobind Vessels / Autobind Lanterns / Autobind Fetters / Autobind Censers / Autobind Thrones / Autobind Pyres / Autobind Urns / Autobind Hearths / Autobind Beacons / Autobind Spires / Autobind Obelisks / Autobind Chalices this-run. Tribute for Favor. Reliquary + Aspects + Vows after first Tribute. The Crown after 2 tributes or 3 Favor earned. Names of the Bound from peak Shades. Remembrance after 3 tributes or 5 Favor earned. Ossuary in Remembrance / The Crown (late Remembrance sink). The Procession in Remembrance / The Crown (this-run Remembrance burst, 45s ×1.2; paid duration `paidProcessionSecs`). The Knell in Remembrance / The Crown (this-run Remembrance click burst, 1 Remembrance, paid duration `paidKnellSecs` ×2 click only). Knell after Tribute if Edict of the Knell (`knellSecs` when level >= 1). Longer Procession in Remembrance (lengthens paid Begin only; edict `processionSecs` stays `45 + 15 * processionEdictLevel`). Deeper Toll in Remembrance (lengthens paid Sound only; edict `tollSecs` stays `25 + 10 * tollEdictLevel`). Longer Wake in Remembrance (lengthens paid Keep the Wake only; edict `wakeSecs` stays `40 + 15 * wakeEdictLevel`). Longer Tithe in Remembrance (lengthens paid Tithe only; Edict of Hymn is not Tithe — hymn stays `45 + 15 * hymnEdictLevel`). Longer Veil in Remembrance (lengthens paid Thin the Veil only; edict `veilSecs` stays `20 + 10 * veilEdictLevel`). Longer Hymn in Remembrance (adds `+10s * n` to Tribute hymn only; edict `hymnSecs` stays `45 + 15 * hymnEdictLevel`; Hymn has no paid button). Longer Knell in Remembrance (lengthens paid Sound only; edict `knellSecs` stays `20 + 10 * knellEdictLevel`). Edict of Night in Reliquary. Edict of the Veil in Reliquary. Edict of the Knell in Reliquary. Edict of the Procession in Reliquary. Urns in the loop as ash half-step after Pyres. Hearths (ash half-step after Urns). Beacons (ash half-step after Hearths). Spires (ash half-step after Beacons). Obelisks (ash half-step after Spires). Edict of Urns in Reliquary. Edict of Hearths in Reliquary. Edict of Beacons in Reliquary. Edict of Spires in Reliquary. Edict of Obelisks in Reliquary. Edict of the Cut in Reliquary. Edict of Kindling (lantern start) in Reliquary. Edict of Tending (hearth autobind) in Reliquary. Edict of the Gleam in Reliquary. Edict of the Rise (spire autobind) in Reliquary.
...
**Buy-mode hotkeys 1/2/3 honor otherButton (AZR-172).** Digit buy-mode keys share the letter-hotkey `!otherButton` guard via `HOTKEYS` table; no steal when another button is focused; `preventDefault` on handled press; AZR-116 hint only on accepted mode switch.

**nextGoal real Favor thresholds (AZR-173).** `FAVOR_SOULS_BASE` / `soulsForFavor` / `nextFavorThreshold`; after first Favor the goal line shows the true next threshold (X/Y), not a hardcoded 25000.

**Ash rites geometric costs (AZR-162).** Flat ash costs on Cinders/Urn/Hearth/Beacon/Spire replaced with `N.cost(base, 2.6, n)`. Bases 22/18/20/24/26. ×1 like Siphon/Levy. Mult curve `1.55^n` unchanged. Binding Toll / Well / ash ladder untouched. Pre-fix high rite levels may leave inflated ash.

**Hidden-tab credit (AZR-163 + AZR-164).** `simulatedUntil` tracks wall ms credited through; advanced only inside `applyDt` by clamped `dt * 1000`. Offline/load and `settleToNow()` use it (not autosave-refreshed `lastTick`). `visibilitychange` / `pagehide` settle + save; return may AZR-115 away-toast. AZR-164 adds a 1Hz `setInterval` heartbeat while hidden (best-effort under browser throttle), `LIVE_FRAME_MAX = 1.0` for Hollow/Autobind live cadence, and Hollow freeze while `document.hidden`.

**Load-failure keep raw (AZR-165).** Corrupt/`!isSaveShape` primary → `loadFailed` + raw retained; autosave gated; bak1 (~1 min) / bak2 (~1 h) offer Restore; Start Fresh confirmed only. No full slots. Hardened `isSaveShape` for finite souls stocks.

**Save sanitise + NaN tripwire (AZR-168).** `loadCount` / `loadNum` clamp every numeric `applySaveData` assignment. `N.load` never returns non-finite or negative (`fromNumber(-1)` remains for internal math). Import rejects malformed core souls; stored negative counts sanitise to 0. ~1/s tripwire reuses the AZR-165 notice and gates autosave; does not wipe LS.

**Shadowed renderNames (AZR-169).** Dead first `renderNames` removed. Live visibility `n >= 1 || namesComplete`; sig `n + ":" + (complete ? "1" : "0")`. Duplicate IIFE function scan fails the build.

**Unlock-card reveal table (AZR-170).** One `REVEALABLE` table drives reveal + hide for all producer unlock cards (includes Spires/Obelisks). `boot` / `adoptSave` / `layTribute` call `revealUnlockedCards(false)` before first paint; render no longer re-reveals via `is-hidden` checks.

**Quiet Court Tribute autobind (AZR-171).** Ash QC autobind (Urns/Hearths/Beacons/Spires/Obelisks) evaluated once after `applyEdictStartingStock`; `TRIBUTE_AUTOBIND_STARTS` table-drives all Tribute autobind starts.

**v6.9 extras.**

**Hollow Hunger (mid-run idle pressure).** Active only when `(favorEarned ≥ 2)` AND `unlockedPyres`. When ungated: freeze the idle timer (no new stacks); existing stacks stay until a qualifying manual spend or Tribute/Reset. LIVE path only (`applyDt(dt, true)` with `liveSpan = min(dt, LIVE_FRAME_MAX)`): visible-tab play, or AZR-164 heartbeat while hidden with Hollow frozen (`document.hidden` → no Hollow accrual). Offline/settle `applyDt(..., false)` must not advance `hollowIdle` or add stacks. Constants: `HOLLOW_GRACE = 90`, `HOLLOW_INTERVAL = 45`, `HOLLOW_MAX = 5`, `HOLLOW_PENALTY = 0.04`. This-run state: `hollowStacks` 0–5, `hollowIdle` seconds since last clear / gate open, `hollowWarned` once-per-emptying toast flag. Stacks from idle: when idle ≥ 90, `want = min(5, 1 + floor((idle - 90) / 45))`; raise stacks to want (never auto-clear from timer). Penalty: `hollowMult(stacks) = 1 - 0.04 * stacks` folded into `rateMult()` with tithe (hits shade souls/s, shades/s, ash, clicks via rateMult). Qualifying manual spend calls `noteHollowManualSpend(kind, spent, stockBefore)` after a successful currency deduct (producers, Marks, rites, paid one-shots, Reliquary/Crown/Remembrance shops) **only if** the spend meets the AZR-148 clear threshold for that currency (souls cap 500; ash/shades floors; other currencies always clear). Sub-threshold buys still succeed but leave stacks and the 90s idle timer alone. **Do not** call from `tryAutobind*` / toggles / `applyDt` gifts / gather click / Aspect-Vow swear. Toast once when first stack lands this emptying: "The well grows hollow." Quiet status `#hollow-status` under souls-rate when stacks ≥ 1 (`Hollow ×N (−Y%)`); hidden at 0. Wipe stacks/idle/warned on Tribute and Footer Reset; old saves → 0 / 0 / false. Hollow **Mark** vessel boost still uses `emberMult(hollowLevel)` (name collision avoided). Autobind does not clear Hollow Hunger.

## Design notes (v6.8)

Visual direction is locked: GodKing / void — near-black oxblood, gold, crimson, bone/cream serif. Do not restyle the well sigil or masthead. Reuse `.rite-row` styles. No new palette.

**v6.8 extras.**

**Binding Toll (this-run ash side path).** Unlock when `N.cmp(fetters, 5) ≥ 0` AND `favorEarned ≥ 1`, or keep visible if `bindingTollLevel ≥ 1` this run. Compact `.rite-row` in Rites after Levy, before Cinders. Spend Ash, cost `N.cost(40, 1.45, n)` (`BINDING_TOLL_COST_BASE` / `BINDING_TOLL_COST_MULT`). ×1 only — ignores buyMode. Cap 4 (`BINDING_TOLL_MAX`). Flavor: *The chain bites both ways.* Button: **Pay the Binding Toll.** Effect: Shade/Spirit rate × `1.12^n` (`BINDING_TOLL_RATE`) on `shadeSoulsPerSec` and `shadesPerSec` only (not vessels, lanterns, clicks, ash producers, or `spiritsPerSec` unless already via shadesPerSec). Shade and Spirit **buy costs** × `(1 + 0.15 × n)` (`BINDING_TOLL_COST_BONUS`) on `shadeCost`/`spiritCost` (Autobind), `purchasePlan`/`bulkCost`/`maxAffordable` optional `extraMult` (manual 1/10/Max), and card cost labels. Vessels/Thrones stay on unboosted `producerCost`. Poverty / Ember / Hunger do not block. State `bindingTollLevel` 0–4 and `unlockedBindingToll` this-run; wipe Tribute (`freshState`) and Footer Reset. Serialize/load; old saves missing → 0 / false. Chronicle: "The binding toll was paid." No Autobind, no Reliquary edict, no hotkey, no new gift. IDs: `binding-toll-row` / `binding-toll-effect` / `binding-toll-cost` / `binding-toll-buy`. Next-goal may quietly hint after Fetters if unlocked and unpaid (does not steal Aspect-swear or Tribute-ready). AZR-117 Well curve and AZR-118 ash-ladder numbers untouched.
...
**Autobind Obelisks (QoL).** Unlock at 3 Obelisks this run (`UNLOCK_AUTOBIND_OBELISKS = 3`). Quiet toggle in Rites after Autobind Spires, before Autobind Chalices. Flavor: *The stone tends itself.* When on, each live tick buys exactly 1 Obelisk if affordable (always ×1, ignores buy-mode; cost is Spires via existing `obeliskCost`). Persist `autobindObelisks` this-run; wipe on Tribute. Offline catchup does not autobind. Autobind Obelisks spends Spires; Autobind Spires may have just bought Spires — same-tick spend is intentional; no extra hold-back vs saving Spires for a manual obelisk. Tick order: after Autobind Spires, before Autobind Chalices. Poverty does not block obelisks or this autobind. No obelisk rite and no obelisk-autobind edict this version. Row gated by unlock count; toggle can be on while hidden if Quiet Court set it. IDs: `autobind-obelisks-row` / effect / buy. Effect on: "The obelisk rises" / off: "Idle bind".

**Quiet Court also starts Autobind Obelisks.** If `quietCourtLevel >= 1` on Tribute meta, also set `autobindObelisks = true` (in addition to shade, lantern, fetter, pyre, chalice, urn, hearth, beacon, and spire autobind). Helper `quietCourtStartsObeliskAutobind(level)` true when level >= 1. Applied next to each existing `quietCourtStartsSpireAutobind` Tribute if (two sites, including after edict-started obelisks) so edict obelisks can unlock the autobind row the same emptying. If obelisks < 3 this emptying, the toggle is on but the row stays hidden until 3 — that's OK; once 3 obelisks, it is already on. Crown Quiet Court effect: Autobind Shades, Lanterns, Fetters, Pyres, Chalices, Urns, Hearths, Beacons, Spires, and Obelisks at tribute.

**Gift: peak obelisks >= 5.** Once when `peakObelisks` (Num.cmp) reaches 5: +7 ash (`Num.add`) (retuned AZR-118). Track `peakObelisks` persist Tribute (already exists from v6.6 — reuse; do not duplicate). Flag `giftPeakObelisks` persist Tribute, wipe Reset. Same flag-then-grant as peak spires (check flag, set, grant, save). Toast: "Seven ash for five obelisks." Chronicle: "Five obelisks. The well returned seven ash." Missing flag grants once if peak already >= 5, then save. Old saves: null → false, then checkUnlock grants once if peak already ≥5.

**Session-layer chronicle.** `giftPeakObelisks`: "Five obelisks. The well returned seven ash."
...
**Obelisks (ash half-step after Spires).** Unlock at 6 Spires this run (`UNLOCK_OBELISKS = 6`) OR owned obelisks ≥ 1 (retuned AZR-118). Hidden until then. Card in the producer row after Spires, before Thrones. Spend Spires, cost `floor(6 × 1.28^n)` via `N.cost` (`OBELISK_COST_BASE = 6`, `OBELISK_COST_MULT = 1.28`). Buy 1 / 10 / Max using existing buyMode + bulkCost. Each Obelisk produces `0.03` Ash/s × `rateMult` × `nightMult` × `hymnMult` × `wakeMult` only — **not** urn/hearth/beacon/spire rites. Additive with Censer, Pyre, Urn, Hearth, Beacon, and Spire ash. Flavor: *Stone after the height.* Button: **Raise an Obelisk**. Effect line: ash/sec from obelisks. State `obelisks` is Num. Persist this-run; wipe Tribute then apply edict. Track `peakObelisks` persist Tribute (peak-5 gift added in v6.7). Poverty does not block (same as Spires / Beacons / Hearths / Urns / Pyres / Censers). First obelisk Chronicle: "An obelisk was raised." Reset wipes. Autobind Obelisks / Quiet Court obelisks / peak-5 gift added in v6.7. No obelisk rite, no obelisk-autobind edict this version. Next-goal may hint after Spires if still unbought (does not steal Aspect-swear or Tribute-ready). Autobind Spires still converts Beacons→Spires with no extra hold-back versus saving Spires for an Obelisk.
...
**Rite of the Spire (this-run).** Unlock when Spires are unlocked (4 Beacons / Spires card visible). Compact row in Rites after Rite of the Beacon, before The Well Draws. Spend ash `N.cost(26, 2.6, n)` via `spireRiteCost(level)` (AZR-162; base 26 from AZR-120; was flat 26). ×1 buy only. Each purchase +1 `spireRiteLevel` (starts 0). Spire portion of Ash/s is multiplied by `spireRiteMult = 1.55^spireRiteLevel` via Num helper `spireRiteMult(level)` (`spireRiteMult(0)=1`, first buy ≈ ×1.55) (retuned AZR-120; mult curve unchanged). Same `siphonMult` curve. Flavor: *The height is doubled.* Button: **Cut the Spire**. Effect: `Spire ×N`. Cost line: `spireRiteCost(spireRiteLevel)` Ash. Poverty does not block (same as beacon rite / spires). Persist `spireRiteLevel` this-run; wipe Tribute; wipe Footer Reset. Chronicle first: "The spire was cut." Does not persist through Tribute. Does **not** apply `beaconRiteMult`/`hearthRiteMult`/`urnRiteMult` to spires or `spireRiteMult` to beacons/hearths/urns/pyres/censers. No extra ash hold-back vs Marks / Night / Veil / Cinders / Chalices / Wake / Beacon rite. No spire-autobind edict this version.
...
**Gift: peak spires >= 5.** Once when `peakSpires` (Num.cmp) reaches 5: +7 ash (`Num.add`) (retuned AZR-118). Track `peakSpires` persist Tribute (already exists from v6.2 — reuse; do not duplicate). Flag `giftPeakSpires` persist Tribute, wipe Reset. Same flag-then-grant as peak beacons (check flag, set, grant, save). Toast: "Seven ash for five spires." Chronicle: "Five spires. The well returned seven ash." Missing flag grants once if peak already >= 5, then save.

**Session-layer chronicle.** `giftPeakSpires`: "Five spires. The well returned seven ash."
...
**Rite of the Beacon (this-run).** Unlock when Beacons are unlocked (4 Hearths / Beacons card visible). Compact row in Rites after Rite of the Hearth, before The Well Draws. Spend ash `N.cost(24, 2.6, n)` via `beaconRiteCost(level)` (AZR-162; base 24 from AZR-120; was flat 24). ×1 buy only. Each purchase +1 `beaconRiteLevel` (starts 0). Beacon portion of Ash/s is multiplied by `beaconRiteMult = 1.55^beaconRiteLevel` via Num helper `beaconRiteMult(level)` (`beaconRiteMult(0)=1`, first buy ≈ ×1.55) (retuned AZR-120; mult curve unchanged). Same `siphonMult` curve. Flavor: *The last light is doubled.* Button: **Cut the Beacon**. Effect: `Beacon ×N`. Cost line: `beaconRiteCost(beaconRiteLevel)` Ash. Poverty does not block (same as hearth rite / beacons). Persist `beaconRiteLevel` this-run; wipe Tribute; wipe Footer Reset. Chronicle first: "The beacon was cut." Does not persist through Tribute. Does **not** apply `hearthRiteMult`/`urnRiteMult` to beacons or `beaconRiteMult` to hearths/urns/pyres/censers. No extra ash hold-back vs Marks / Night / Veil / Cinders / Chalices / Wake / Hearth rite. No beacon-autobind edict this version.
...
**Gift: peak beacons >= 5.** Once when `peakBeacons` (Num.cmp) reaches 5: +7 ash (`Num.add`) (retuned AZR-118). Track `peakBeacons` persist Tribute (already exists from v5.8 — reuse; do not duplicate). Flag `giftPeakBeacons` persist Tribute, wipe Reset. Same flag-then-grant as peak hearths (check flag, set, grant, save). Toast: "Seven ash for five beacons." Chronicle: "Five beacons. The well returned seven ash." Missing flag grants once if peak already >= 5, then save.

**Session-layer chronicle.** `giftPeakBeacons`: "Five beacons. The well returned seven ash."
...
**Rite of the Hearth (this-run).** Unlock when Hearths are unlocked (4 Urns / Hearths card visible). Compact row in Rites after Rite of the Urn, before The Well Draws. Spend ash `N.cost(20, 2.6, n)` via `hearthRiteCost(level)` (AZR-162; base 20 from AZR-120; was flat 20). ×1 buy only. Each purchase +1 `hearthRiteLevel` (starts 0). Hearth portion of Ash/s is multiplied by `hearthRiteMult = 1.55^hearthRiteLevel` via Num helper `hearthRiteMult(level)` (`hearthRiteMult(0)=1`, first buy ≈ ×1.55) (retuned AZR-120; mult curve unchanged). Same `siphonMult` curve. Flavor: *The last heat is doubled.* Button: **Cut the Hearth**. Effect: `Hearth ×N`. Cost line: `hearthRiteCost(hearthRiteLevel)` Ash. Poverty does not block (same as urn rite / hearths). Persist `hearthRiteLevel` this-run; wipe Tribute; wipe Footer Reset. Chronicle first: "The hearth was cut." Does not persist through Tribute. Does **not** apply `urnRiteMult` to hearths or `hearthRiteMult` to urns/pyres/censers. No extra ash hold-back vs Marks / Night / Veil / Cinders / Chalices / Wake. No hearth-autobind edict this version.
...
**Rite of the Urn (this-run).** Unlock when Urns are unlocked (4 Pyres / Urns card visible). Compact row in Rites with Siphon/Levy/Cinders. Spend ash `N.cost(18, 2.6, n)` via `urnRiteCost(level)` (AZR-162; base 18 from AZR-120; was flat 18). ×1 buy only. Each purchase +1 `urnRiteLevel` (starts 0; do **not** name this `vesselLevel` — that collides with the Vessels producer). Urn portion of Ash/s is multiplied by `urnRiteMult = 1.55^urnRiteLevel` via Num helper `urnRiteMult(level)` (`urnRiteMult(0)=1`, first buy ≈ ×1.55) (retuned AZR-120; mult curve unchanged). Flavor: *The vessel is doubled.* Button: **Cut the Urn**. Cost line: `urnRiteCost(urnRiteLevel)` Ash. Poverty does not block (same as Cinders / Siphon). Persist `urnRiteLevel` this-run; wipe Tribute. Chronicle first: "The urn was cut." Does not persist through Tribute.
...
**Chalices (late ash sink).** Unlock at 5 Thrones this run (`UNLOCK_CHALICES = 5`). Hidden until then. Card in the producer row after Thrones. Spend Ash, cost `floor(32 × 1.65^n)` via `N.cost` (retuned AZR-118; was 20 × 1.5). Buy 1 / 10 / Max using existing buyMode + bulkCost. Cap at 12 (`CHALICE_MAX = 12`). Each Chalice: `chaliceMult(n) = 1 + 0.08 * n`, folded into `prodMult` as an extra multiplicative factor `(1 + 0.08 * chalices)`. Flavor: *He drinks from the emptied well.* Button: **Raise a Chalice**. Effect: +8% production each. Persist this-run; wipe Tribute then apply edict. Reset wipes. First chalice Chronicle: "A chalice was raised." Poverty: follow Marks/Censers (do not block). Next-goal may hint after Thrones if still unbought (does not steal Aspect-swear or Tribute-ready).
...
**Rite of Cinders (this-run).** Unlock when Pyres are unlocked (3 Censers / Pyres card visible). Compact row in Rites with Siphon/Levy. Spend ash `N.cost(22, 2.6, n)` via `cinderCost(level)` (AZR-162; base 22 from AZR-120; was flat 22). ×1 buy only (like Siphon/Levy — not Buy 10/Max). Each purchase +1 `cinderLevel` (starts 0). Pyre portion of Ash/s is multiplied by `cinderMult = 1.55^cinderLevel` via Num helper `cinderMult(level)` (`cinderMult(0)=1`, first buy ≈ ×1.55) (retuned AZR-120; mult curve unchanged). Flavor: *The pyre is doubled.* Button: **Cut the Cinders**. Cost line: `cinderCost(cinderLevel)` Ash. Poverty does not block (same as Siphon). Persist `cinderLevel` this-run; wipe Tribute. Chronicle first cinders: "The cinders were cut." Does not persist through Tribute.
...
Toasts are queued (array, AZR-174). Cap `TOAST_QUEUE_MAX = 5` with `…and N more.` overflow; same-tick milestone gifts coalesce to one summary; click/Escape advances (Escape again clears); fast drain 1.8s when more wait, else 5.2s. Away-harvest (AZR-115) still leads the queue. Chronicle still records every gift.
...
**Loop.** Click the well. Shades (base 10 souls, ×1.15, 1 soul/s). Bound Spirits at 10 shades or 100 lifetime souls (0.1 shade/s). Well Depth after 1 shade (+1 click; early d≤5 `floor(25×1.35^d)`, else `floor(25×1.5^d)` souls). Vessels at 5 spirits or 50 lifetime shades (0.1 spirit/s). Thrones at 1 vessel or 50 lifetime spirits this run (cost in vessels; +10% production each, or +15% under Dominion). Chalices at 5 Thrones this run (late ash sink; cost Ash `floor(32×1.65^n)`; cap 12; +8% production each). Buy 1 / 10 / Max on main-line producers, Pyres, Urns, and Chalices (not Well Depth, not Lanterns, not Fetters, not Censers, not Rites, not Marks, not Aspects, not the Tithe, not Night's Tithe, not The Wake, not Thin the Veil, not Choir of Ash, not The Crown, not Vows). Tick: souls from shades, shades from spirits, spirits from vessels, Ash from shade souls, Censers, Pyres, and Urns. Lanterns at 3 Shades; Fetters at 3 Bound Spirits; Censers at 1 Vessel or 25 lifetime Spirits; Pyres at 3 Censers (ash half-step); Urns at 6 Pyres (ash half-step after Pyres; retuned AZR-118); Chalices at 5 Thrones.
...
**Rites (this-run).** Hidden until the first Shade this run, or if The Well Draws is already on from Edict of Echoes. Forgotten on Tribute and on Footer Reset. ×1 buy only. Autobind Lanterns and Thin the Veil live here (v1.6). Autobind Fetters lives here (v1.8). Autobind Censers lives here (v1.9). Autobind Thrones lives here (v2.0). Autobind Pyres lives here (v2.2). Autobind Chalices lives here (v2.6). Autobind Urns lives here (v3.8). Autobind Hearths lives here (v5.1). Autobind Beacons lives here (v5.9). Autobind Spires lives here (v6.3). Autobind Obelisks lives here (v6.7). Rite of Cinders lives here with Siphon/Levy (v2.2). Rite of the Urn lives here with Siphon/Levy/Cinders (v3.9). Rite of the Hearth lives here with Siphon/Levy/Cinders/Urn (v5.2). Rite of the Beacon lives here with Siphon/Levy/Cinders/Urn/Hearth (v6.0). Rite of the Spire lives here with Siphon/Levy/Cinders/Urn/Hearth/Beacon (v6.4). Binding Toll lives here after Levy, before Cinders (v6.8).

- Rite of Siphon: each level multiplies Shade soul output by `1.55^level` (retuned AZR-120; was ×2^level). Cost `floor(65×3^level)` Souls (was 50).
- Rite of Levy: each level multiplies Bound Spirit shade output by `1.55^level` (retuned AZR-120; was ×2^level). Hidden until Bound Spirits. Cost `floor(22×3^level)` Shades (was 15).
- Rite of Cinders: each level multiplies Pyre ash by `1.55^level` (AZR-120). Cost `N.cost(22, 2.6, n)` Ash (AZR-162; was flat). ×1 only. Poverty does not block. Wipe on Tribute.
- Rite of the Urn: each level multiplies Urn ash by `1.55^level`. Hidden until Urns unlocked. Cost `N.cost(18, 2.6, n)` Ash (AZR-162). ×1 only. Poverty does not block. Wipe on Tribute. State `urnRiteLevel` (not `vesselLevel`).
- Rite of the Hearth: each level multiplies Hearth ash by `1.55^level`. Hidden until Hearths unlocked. Cost `N.cost(20, 2.6, n)` Ash (AZR-162). ×1 only. Poverty does not block. Wipe on Tribute. State `hearthRiteLevel`.
- Rite of the Beacon: each level multiplies Beacon ash by `1.55^level`. Hidden until Beacons unlocked. Cost `N.cost(24, 2.6, n)` Ash (AZR-162). ×1 only. Poverty does not block. Wipe on Tribute. State `beaconRiteLevel`.
- Rite of the Spire: each level multiplies Spire ash by `1.55^level`. Hidden until Spires unlocked. Cost `N.cost(26, 2.6, n)` Ash (AZR-162). ×1 only. Poverty does not block. Wipe on Tribute. State `spireRiteLevel`.
...
**Multiplier.** `prodMult = prestigeMult(favorEarned) * (1 + throneWeight * thrones) * (1 + 0.25 * edictLevel) * crownProdFactor(crownWeight) * namesCompleteMult * (1 + 0.08 * chalices) * (1 + 0.05 * ossuaryLevel)`, where `prestigeMult = 1.02^favor * (1 + 0.2 * min(favor, 4))`, `crownProdFactor = 1.1^crownWeight`, `throneWeight` 0.15 if Dominion else 0.10, `namesCompleteMult` 1.5 if all twelve Names are bound else 1, `chalices` 0–12 this run, `ossuaryLevel` 0–8 persist Tribute. Live rates use `currentMult = prodMult * processionMult`, `processionMult` 1.2 while `processionLeft > 0` else 1 (this-run; wipe Tribute).

**Prestige shape (AZR-181 option 2).** Favor lowers the Shade/Spirit/Vessel/Throne/Censer cost multiplier: `producerCostMult(favor) = 1.15 → 1.06` via `1 - exp(-favor/40)`. Soft compound `prestigeMult` keeps deep Favor from asymptoting. Crown costs are classic `×2` through level 8, then linear Favor add; Crown bonus is `1.1^n` so the track stays worth buying.
...
shade souls/s = `shades * 1 * prodMult * titheMult * 1.55^siphon * harvestMult * lanternMult * emberMult * nightMult * hymnMult` (+ `clickPower` if `wellDraws`) (retuned AZR-120). `harvestMult` is 1.5 if Harvest else 1. `hymnMult` is 1.25 while `hymnLeft > 0` else 1.

shades/s from spirits = `spirits * 0.1 * prodMult * titheMult * 1.55^levyLevel * bindingMult * chainMult * fetterMult * hymnMult` (retuned AZR-120). `bindingMult` is 1.5 if Binding else 1.
