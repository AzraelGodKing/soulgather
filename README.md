...
**Vows remembered (cross-emptying collection).** Swearing different Vows across runs is a late goal, not only the next producer. When a vow is sworn (stillness / poverty / hunger / ember), persist a flag for that id (`vowsKnown.stillness` etc.). Persist through Tribute; wipe Footer Reset. Helper `vowsKnownCount(known)` returns 0–4. Quiet Chronicle stats line when count >= 1: `Vows remembered: n / 4` (same style as Names bound). Does not steal Aspect-swear or Tribute-ready next-goal. Old saves: seed from Chronicle vow lines if those exist (first vow sworn / this-run `vow`, ember vow `vowEmber` / `giftFirstEmberVow`) without granting the all-four gift yet.
