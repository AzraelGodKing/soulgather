...
**Gift: peak obelisks >= 5.** Once when `peakObelisks` (Num.cmp) reaches 5: +12 ash (`Num.add`). Track `peakObelisks` persist Tribute (already exists from v6.6 — reuse; do not duplicate). Flag `giftPeakObelisks` persist Tribute, wipe Reset. Same flag-then-grant as peak spires (check flag, set, grant, save). Toast: "Twelve ash for five obelisks." Chronicle: "Five obelisks. The well returned twelve ash." Missing flag grants once if peak already >= 5, then save. Old saves: null → false, then checkUnlock grants once if peak already ≥5.

**Session-layer chronicle.** `giftPeakObelisks`: "Five obelisks. The well returned twelve ash."
...
**Obelisks (ash half-step after Spires).** Unlock at 4 Spires this run (`UNLOCK_OBELISKS = 4`) OR owned obelisks ≥ 1. Hidden until then. Card in the producer row after Spires, before Thrones. Spend Spires, cost `floor(6 × 1.2^n)` via `N.cost` (`OBELISK_COST_BASE = 6`, `OBELISK_COST_MULT = 1.2`). Buy 1 / 10 / Max using existing buyMode + bulkCost. Each Obelisk produces `0.03` Ash/s × `rateMult` × `nightMult` × `hymnMult` × `wakeMult` only — **not** urn/hearth/beacon/spire rites. Additive with Censer, Pyre, Urn, Hearth, Beacon, and Spire ash. Flavor: *Stone after the height.* Button: **Raise an Obelisk**. Effect line: ash/sec from obelisks. State `obelisks` is Num. Persist this-run; wipe Tribute then apply edict. Track `peakObelisks` persist Tribute (peak-5 gift added in v6.7). Poverty does not block (same as Spires / Beacons / Hearths / Urns / Pyres / Censers). First obelisk Chronicle: "An obelisk was raised." Reset wipes. Autobind Obelisks / Quiet Court obelisks / peak-5 gift added in v6.7. No obelisk rite, no obelisk-autobind edict this version. Next-goal may hint after Spires if still unbought (does not steal Aspect-swear or Tribute-ready). Autobind Spires still converts Beacons→Spires with no extra hold-back versus saving Spires for an Obelisk.
