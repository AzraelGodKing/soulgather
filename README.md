...
**Hollow Hunger (mid-run idle pressure).** Active only when `(favorEarned ≥ 2)` AND `unlockedPyres`. When ungated: freeze the idle timer (no new stacks); existing stacks stay until a qualifying manual spend or Tribute/Reset. LIVE `applyDt(dt, true)` only — same live path as Autobind; offline catchup must not advance `hollowIdle` or add stacks. Constants: `HOLLOW_GRACE = 90`, `HOLLOW_INTERVAL = 45`, `HOLLOW_MAX = 5`, `HOLLOW_PENALTY = 0.04`. This-run state: `hollowStacks` 0–5, `hollowIdle` seconds since last clear / gate open, `hollowWarned` once-per-emptying toast flag. Stacks from idle: when idle ≥ 90, `want = min(5, 1 + floor((idle - 90) / 45))`; raise stacks to want (never auto-clear from timer). Penalty: `hollowMult(stacks) = 1 - 0.04 * stacks` folded into `rateMult()` with tithe (hits shade souls/s, shades/s, ash, clicks via rateMult). Qualifying manual spend calls `noteHollowManualSpend()` after a successful currency deduct (producers, Marks, rites, paid one-shots, Reliquary/Crown/Remembrance shops). **Do not** call from `tryAutobind*` / toggles / `applyDt` gifts / gather click / Aspect-Vow swear. Toast once when first stack lands this emptying: "The well grows hollow." Quiet status `#hollow-status` under souls-rate when stacks ≥ 1 (`Hollow ×N (−Y%)`); hidden at 0. Wipe stacks/idle/warned on Tribute and Footer Reset; old saves → 0 / 0 / false. Hollow **Mark** vessel boost still uses `emberMult(hollowLevel)` (name collision avoided). Autobind does not clear Hollow Hunger.
